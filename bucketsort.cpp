...
vector<int> bucketSort(vector<int>& v) {             // Using Linked Lists as buckets and STL quicksort for sorting the buckets
    ...
    double min = *min_element(v.begin(), v.end());   // Using min and max to estimate the range for the buckets/bins
    int bucketSize = ceil((max - min)/numBuckets);   // The distance between max and min divided by num of buckets
    ...
    for (int i = 0; i < v.size(); i++) {             // Iterating over the array to be sorted for sending elems to buckets
        int j = floor((v[i] - min)/bucketSize);      // j-index is the specific bucket to which elem is sent
        ...
        if (j < v.size())                            // IF is required in divider based approach or if max and min are ints
            B[j].push_back(v[i]);                    // Usually pushing to j-index in the vector of lists works
    ...
    for (int j = 0; j < B.size(); j++) {             // Sort all buckets internally stored in the vector B
        B[j].sort();                                 // std::list::sort() is more approach for sorting lists

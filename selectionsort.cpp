...
void selectionSort(vector<int>& v) {
    for (int i = 0; i < v.size() - 1; i++) {         // Iterate over the array v.size() - 2 times (last already sorted)
        ...
        for (int j = i; j < v.size(); j++) {         // Start with the first elem in the unsorted subarray and go to the end
            if (v[j] < v[minIndex])                  // If any element is less than the current min elem
                minIndex = j;                        // Update the min index to point to the new min element
    ...
    }                                                // Repeat this process once for all v.size() - 1 elements
}
